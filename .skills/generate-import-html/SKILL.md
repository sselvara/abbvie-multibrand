...
**Brand and Theme (from page-import Step 0.5):**
- If `BRAND` is set (non-empty) → Include as `brand` property with the brand name value
- If `THEME` is set (non-empty) → Include as `theme` property with the theme name value
- If both are empty → Omit both (page loads with default global styles)

These two properties are read by `scripts/multi-theme.js` at runtime to load the correct CSS cascade:
- `brand` → loads `styles/{brand}/styles.css`, `styles/{brand}/tokens.css`, `styles/{brand}/fonts.css`, and `blocks/{block}/{brand}/{block}.css` for each block
- `theme` → additionally loads `styles/{brand}/themes/{theme}/` overrides and `blocks/{block}/{brand}/themes/{theme}/{block}.css`

Without these metadata fields, the page renders with the global default styles regardless of which blocks are on the page.
...
<div>
  <div class="metadata">
    ...
    <!-- Include brand if BRAND is set (e.g. "roy") -->
    <div>
      <div>brand</div>
      <div>roy</div>
    </div>
    <!-- Include theme if THEME is set (e.g. "bright") -->
    <div>
      <div>theme</div>
      <div>bright</div>
    </div>
...
**Brand and theme must be last** in the metadata block (after title/description/image) so the platform processes them after content metadata.
...
Apply `CONTENT_PATH_PREFIX` from page-import Step 0.5 to both the HTML path and images path:

- Brand `roy`, HTML: `products/widget.plain.html` → Images at: `roy/products/images/`
- No brand, HTML: `us/en/about.plain.html` → Images at: `us/en/images/`
- No brand, HTML: `index.plain.html` → Images at: `images/`

**Rule:** Images folder goes in the same directory as the (prefix-adjusted) HTML file.

**Step 2: Copy the images folder**

```bash
# Example: Brand roy, source path products/widget.plain.html
mkdir -p roy/products/images
cp -r ./import-work/images/* roy/products/images/

# Example: No brand, source path us/en/about.plain.html
...
The HTML references images as `./images/...` which is correct for files in the same directory. No path changes needed in the HTML itself.

**Example:**
```
HTML location: roy/products/widget.plain.html
Images location: roy/products/images/
Image reference in HTML: <img src="./images/abc123.jpg">
Result: ✅ Correct - browser resolves to roy/products/images/abc123.jpg
```
...
**Determine the output path** using both `paths.htmlFilePath` from metadata.json and `CONTENT_PATH_PREFIX` from page-import Step 0.5:

```
outputPath = {CONTENT_PATH_PREFIX}/{paths.htmlFilePath}
```

**Examples:**
- Brand `roy`, source path `products/widget.plain.html` → save to `roy/products/widget.plain.html`
- Brand `rapidly`, source path `index.plain.html` → save to `rapidly/index.plain.html`
- No brand (default), source path `us/en/about.plain.html` → save to `us/en/about.plain.html`

**Images folder** follows the same rule — place it alongside the HTML file:
- Brand page: `{brand}/{source-dir}/images/` (e.g. `roy/products/images/`)
- Default page: `{source-dir}/images/`

Read the metadata.json file from scrape-webpage to get `paths.htmlFilePath`, then prepend `CONTENT_PATH_PREFIX`.

**Create the brand directory if it doesn't exist:**
```bash
mkdir -p {CONTENT_PATH_PREFIX}/{paths.dirPath}
```
...
- ✅ HTML file saved: HTML file written to disk at `{CONTENT_PATH_PREFIX}/{paths.htmlFilePath}`
- ✅ Images folder copied: Images folder exists at `{CONTENT_PATH_PREFIX}/{paths.dirPath}/images/`
- ✅ Images accessible: Verify that at least one image file exists in the copied images folder
- ✅ Brand metadata: If BRAND is set, `brand` property appears in the metadata block
- ✅ Theme metadata: If THEME is set, `theme` property appears in the metadata block
- ✅ Path is brand-prefixed: If BRAND is set, the file lives under `{brand}/` not at the root source path
