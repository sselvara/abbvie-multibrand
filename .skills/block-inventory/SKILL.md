...
**If a BRAND is active (passed from page-import Step 0.5):**

For each local block, check for brand-level overrides:

```bash
# Check if this brand has overrides for the block
ls blocks/{block-name}/{brand}/
# e.g. ls blocks/hero/roy/
```

Note the following for the block inventory output:
- **Brand CSS override** — `blocks/{block}/{brand}/_{block}.css` exists → block has brand-specific visual treatment
- **Brand config override** — `blocks/{block}/{brand}/block-config.js` exists → may add brand-specific variations or flags. Read it to find additional `variations[]` entries beyond the root `block-config.js`
- **Brand+theme override** — `blocks/{block}/{brand}/themes/{theme}/` exists → block has combined brand+theme visual treatment

This tells authoring-analysis which CSS class variations are actually available for this brand/theme combination (not just what's in the base `block-config.js`).

**Output:** List of local block names with brand-level override notes
...
node .skills/block-collection-and-party/scripts/search-block-collection-github.js quote &
node .skills/block-collection-and-party/scripts/search-block-collection-github.js fragment &
wait
```
...
LOCAL BLOCKS:
- {block-name}: {purpose}
  Brand overrides (brand: {BRAND}): CSS override ✅ | Config override ✅ (extra variations: ['split', 'wide'])
- {block-name}: {purpose}
  Brand overrides (brand: {BRAND}): none
...
- Brand-level overrides are applied automatically by `multi-theme.js` — no extra authoring steps needed; they affect which visual variants are available
